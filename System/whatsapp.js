...
exports.serialize = (WhaX, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && WhaX.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(WhaX.user && WhaX.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        WhaX.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        WhaX.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => WhaX.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? WhaX.sendFile(chatId, text, "file", "", m, { ...options })
      : WhaX.sendText(chatId, text, m, { ...options });
