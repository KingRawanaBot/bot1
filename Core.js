...
module.exports = async (WhaX, m, commands, chatUpdate) => {
  try {
    ...
    const metadata = m.isGroup ? await WhaX.groupMetadata(from) : {};
    ...
    const groupAdmin = m.isGroup
      ...
      : [];
    const botNumber = await WhaX.decodeJid(WhaX.user.id);
    ...
    global.suppL = "https://chat.whatsapp.com/JREQ0z0f95A0IES0fQDHfq";
    ...
    async function doReact(emoji) {
      ...
      await WhaX.sendMessage(m.from, reactm);
    }
    ...
    //if (body.startsWith(prefix) && !icmd)  return WhaX.sendMessage(m.from, { text: "Baka no such command" });
    ...
    if (isAntilinkOn && m.isGroup && !isAdmin && !isCreator && isBotAdmin) {
      const linkgce = await WhaX.groupInviteCode(from);
    ...
    let upTxt = `〘  ${botName} Official Edition  〙    ⚡ Uptime: ${uptime()}`;
    WhaX.setStatus(upTxt);
