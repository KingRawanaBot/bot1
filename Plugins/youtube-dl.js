...
module.exports = {
  ...
  start: async (WhaX, m, { inputCMD, text, doReact, prefix, pushName }) => {
    switch (inputCMD) {
      case "play":
      case "song":
        if (!text) {
        ...
        await doReact("📥");
        thumbWhaX = "https://graph.org/file/d0a287fa875c809f234ce.jpg";
...
_🏮 Video Uploaded:_ *${song.ago}*\n`,
        ...
        YT.mp3(videoId).then((file) => {
          ...
          ffmpeg(inputPath)
            ...
            .on("end", async () => {
              await WhaX.sendPresenceUpdate("recording", m.from);
...
_🏮 Video Uploaded:_ *${song.ago}*\n`,
        ...
        YT.mp3(videoId).then((file) => {
          ...
          ffmpeg(inputPath)
            ...
            .on("end", async () => {
              const thumbnailBuffer = await getBuffer(thumbWhaX);
...
_🏮 Video Uploaded:_ *${song.ago}*\n`,
      ...
      case "ytsearch":
        ...
        WhaX.sendMessage(m.from, buttonMessage, { quoted: m });
        break;
